...
structure PagingParams {
    ...
    /// - desc: Start from the end of the timeline (newest). The first request returns the newest N items (e.g. [4, 5, 6]).
...
structure PageMetadata {
    ...
    /// Only needed for complex pagination; for simple pagination the client can derive the cursor from the result list.
    /// A string that can be used to retrieve the next page of results by passing the value as the starting_after parameter to the same endpoint.
    @required
    next_cursor: String
}
